...
const utimesAsync = BB.promisify(fs.utimes);
...
function refresh (cache, key) {
  const bucket = bucketPath(cache, key)
  const time = new Date();
  return utimesAsync(bucket, time, time).catch(nop);
}

module.exports.refresh.sync = refreshSync;
function refreshSync (cache, key) {
  const bucket = bucketPath(cache, key)
  try {
    const time = new Date();
    fs.utimesSync(bucket, time, time)
  } catch(err) {
    //
  }
}
...
function find (cache, key) {
  const bucket = bucketPath(cache, key)
  return statAsync(bucket).then(stat => {
    return bucketEntries(bucket).then(entries => {
      return entries.reduce((latest, next) => {
        if (next && next.key === key) {
          return formatEntry(cache, next, stat)
        } else {
          return latest
        }
      }, null)
...
function formatEntry (cache, entry, stat) {
